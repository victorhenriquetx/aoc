...
def solve(coords, size_x, size_y):
	...
	for coord in coords:
		...
		if dx == 0:
			...
			#(2,0) -> (6,4)
			if dx > 0:
				new_points = [(x,y) for x in range(x0, x1+1) for y in range(y0, y1+1) if x-y==x0-y0]
				points+= new_points
			if dx < 0:
				# (8,8) -> (0,0)
				# (6,4) -> (2,0)
				new_points = [(x,y) for x in range(x1, x0+1) for y in range(y1, y0+1) if x-y==x0-y0]
				points+= new_points
		elif dx == -dy:
			#diagonal down left
			# (0,8)->(8,0)
			# (5,5)->(8,2)
			if dx > 0:
				new_points = [(x,y) for x in range(x0, x1+1) for y in range(y1, y0+1) if x+y==x0+y0]
				points+= new_points
			# (8,0)->(0,8)
			# (8,2)->(5,5)
			if dx < 0:
				new_points = [(x,y) for x in range(x1, x0+1) for y in range(y0, y1+1) if x+y==x0+y0]
